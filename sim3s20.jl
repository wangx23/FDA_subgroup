...
@everywhere function sim3s30(seed)
    ...
    res12 = GrFDA(indexy,tm,y,knots,2,wt,betam0v5,
    lam = lamvec[9],
    K0=12,maxiter = 1000)

    res13 = GrFDA(indexy,tm,y,knots,1,wt,betam0v5,
    lam = lamvec[15],
    K0=12,maxiter = 1000)


    n*obj.lent*log(res1.residsum/n/obj.lent) +
    Cn*log(n*obj.lent) *(1*p)/(n*obj.lent) + 2*n*(2*p - 3)

    n*obj.lent*log(res13.residsum/n/obj.lent) +
    Cn*log(n*obj.lent) *(1*p)/(n*obj.lent) + 2*n*(1*p - 1)


    n*obj.lent*log(res12.residsum/n/obj.lent) +
    Cn*log(n*obj.lent) *(3*p)/(n*obj.lent) + 2* n *(2*p - 3)


    log(res12.residsum/n/obj.lent) +
    Cn*log(n)*(3*p + 2*p - 3)/n
    ...
    wt2 = exp.(alp2[2] .* (1 .- ordermat[findall(tril(ordermat).!=0)]))
    res2 = GrFDA(indexy,tm,y,knots,2,wt2,betam0v5,
    lam = lamvec[20],
    K0=12,maxiter = 1000)
...
writedlm("resultsim3s30.csv", resultsim3s30, ',')
