...
data4s30 = zeros(14*14*30,5,100)

for i = 1:100
    datai = simdat4s(sig2, lamj, group, m = m,seed = i)
    data4s30[:,1,i] = datai.group
    data4s30[:,2,i] = datai.ind
    data4s30[:,3,i] = datai.time
    data4s30[:,4,i] = datai.obs
    data4s30[:,5,i] = datai.meanfun
end


using RCall
@rput data4s30
R"saveRDS(data4s30, '../simdata/data4s30.rds')"
