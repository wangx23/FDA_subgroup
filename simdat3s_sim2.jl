...
function simdat3s_sim2(sig2::Number, lamj::Vector, group::Vector;
    ...
    rande = sqrt(sig2) * randn(rng,ntotal)
    ...
    ### group 1
    for i= 1:ng1
        vi = sqrt(lamj[1])*randn(rng,1)[1]*psi2.(tvec) +
                sqrt(lamj[2])*randn(rng,1)[1]*psi3.(tvec)
