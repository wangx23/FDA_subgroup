...
@everywhere function sim4s30(seed::Int64)
    ...
    lamvec = collect(range(0.2,0.6,step = 0.01))
    nlam = length(lamvec)
    ...
    rmse1 = norm(res1.meanfunest- meanfun)/sqrt(nobstotal*30)
    estpc1 = index1[2]
    ...
    #lamvec2 = collect(range(0.2,0.7,length = 41))
    #nlam2 = length(lamvec2)
    #lam2 = zeros(nlam2,nalp)
    #lam2[:,1] = lamvec2
    #lam2[:,2] = collect(range(0.3,2,length=41))
    #lam2[:,3] = collect(range(0.5,5,length=41))

    lamvec2 = collect(range(0.3,0.8,length = 21))
    nlam2 = length(lamvec2)
    lam2 = zeros(nlam2,nalp)
    lam2[:,1] = lamvec2
    lam2[:,2] = collect(range(0.3,0.8,length = 21))
    lam2[:,3] = collect(range(0.3,0.8,length = 21))

    BICvec2 = zeros(nlam2,3,nalp)

    for l1 = 1:nalp
    wt2 = exp.(alp2[l1] .* (1 .- ordermat[findall(tril(ordermat).!=0)]))
    for P = 1:3
        for l = 1:nlam2
            res1l = GrFDA(indexy,tm,y,knots,P,wt2,betam0v5,lam = lam2[l,l1], nu = 1,
            K0=12,maxiter = 500)
    ...
    res2 = GrFDA(indexy,tm,y,knots,index2[2],wt2,betam0v5,nu = 1,
    lam = lam2[index2[1],index2[3]],
    K0=12,maxiter = 500)
    ...
    estpc2 = index2[2]
    rmse2 = norm(res2.meanfunest - meanfun)/sqrt(nobstotal*30)
...
resultsim4s30 = pmap(sim4s30, 1:100)
writedlm("../resultnew_v4/resultsim4s30.csv", resultsim4s30, ',')
