...
function GrFDA(indexy::Vector, tm::Vector, y::Vector, knots::Vector,
    ...
    tolabs::Number = 1e-4, tolrel::Number = 1e-2, seed::Int = 123456)
    ...
    n_init = 200
    sse = zeros(n_init)
    groupkmat = zeros(Int,n, n_init)
    # Run KMeans multiple times
    betamt = transpose(betam0)
    for i in 1:n_init
        reskmi = kmeans(betamt, K0; maxiter = 200, rng = rng)
        sse[i] = sum((reskmi.centers[:,reskmi.assignments] .- betamt).^2)
        groupkmat[:,i] = reskmi.assignments
    end



    #reskm = kmeans(transpose(betam0), K0; maxiter = 200, rng = rng)
    #reskm = kmeans(transpose(betam0), K0; maxiter = 200)
    #groupkm = reskm.assignments
    #centerskm = reskm.centers

    groupkm = groupkmat[:,argmin(sse)]
    ...
    lamj = lamj, lamjold = lamjold, sig20 = sig20, mhat = mhat, Vi = Vi,
    residsum = residsum, lent = lent, deltam = deltam, rvalue = rvalue, svalue = svalue,
    tolpri = tolpri, toldual = toldual, niteration = niteration, flag = flag, groupkm = groupkm)
