...
data3s30_v4 = zeros(4320,5,100)

for i = 1:100
    datai = simdat3s_v4(sig2, lamj, group, m = m,seed = i)
    data3s30_v4[:,1,i] = datai.group
    data3s30_v4[:,2,i] = datai.ind
    data3s30_v4[:,3,i] = datai.time
    data3s30_v4[:,4,i] = datai.obs
    data3s30_v4[:,5,i] = datai.meanfun
end


using RCall
@rput data3s30_v4
R"saveRDS(data3s30_v4, '../simdata/data3s30_v4.rds')"
