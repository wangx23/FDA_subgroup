...
data3s10_v5 = zeros(1440,5,100)

for i = 1:100
    datai = simdat3s_v5(sig2, lamj, group, m = m,seed = i)
    data3s10_v5[:,1,i] = datai.group
    data3s10_v5[:,2,i] = datai.ind
    data3s10_v5[:,3,i] = datai.time
    data3s10_v5[:,4,i] = datai.obs
    data3s10_v5[:,5,i] = datai.meanfun
end


@rput data3s10_v5
R"saveRDS(data3s10_v5, '../simdata/data3s10_v5.rds')"
