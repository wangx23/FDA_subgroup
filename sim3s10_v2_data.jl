...
data3s10_v2 = zeros(1440,5,100)

for i = 1:100
    datai = simdat3s(sig2, lamj, group, m = m,seed = i)
    data3s10_v2[:,1,i] = datai.group
    data3s10_v2[:,2,i] = datai.ind
    data3s10_v2[:,3,i] = datai.time
    data3s10_v2[:,4,i] = datai.obs
    data3s10_v2[:,5,i] = datai.meanfun
end


using RCall
@rput data3s10_v2
R"saveRDS(data3s10_v2, '../simdata/data3s10_v2.rds')"