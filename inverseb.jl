...
##### include the psline penalty part
function inversebv2(indexy::Vector, Bm::Array, nu::Number,
  p::Int, n::Int, np::Int, Dm2::Array, lam1::Number)
  ...
  for i = 1:n
    indexi = indexy.== uindex[i]
    temp = vcat(Bm[indexi,:], sqrt(lam1)*Dm2)
    mati = inv(transpose(temp) * temp  + nIp)
    DB = DB + mati
    AB[idp1:idp2,:] = mati
    matinv[idp1:idp2,idp1:idp2] = mati
    idp1 = idp1 + p
